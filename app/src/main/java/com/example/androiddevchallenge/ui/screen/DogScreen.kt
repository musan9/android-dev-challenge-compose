...
sealed class Screen(open val type : ScreenType) {
    ...
    data class Details(val info : DogInfo, override val type : ScreenType = ScreenType.Details) : Screen(type)
}
@Composable
fun DogScreen(onBack : OnBackPressedDispatcher, owner : LifecycleOwner) {
    ...
    when(currentScreen.value.type) {
        ...
        ScreenType.Details -> {
            DogDetailsScreen(info = (currentScreen.value as? Screen.Details)?.info?:return)
        }
    }
}